...
if [ "$#" -eq  "0" ]
then
    echo "No arguments supplied; will use max size"
elif [ "$#" -eq  "1" ]
then
    echo "Reading value from portal"
    RESIZEROOT=$1
else
    echo "ERROR: More than 1 argument supplied"
    exit 0
fi
echo "RESIZEROOT value is $RESIZEROOT"

if [ -z "$RESIZEROOT" ]; then
    logtend "ERROR: must define RESIZEROOT to the new total size (GB) you want for the rootfs!"
    exit 0
fi
if [ -z "$IMPOTENT" ]; then
    IMPOTENT=0
